...
import { intializeItems, intializeVendor } from '../PickList/intializeProperties';
import { formatDate, formatNum, formatDateFromDB } from '../../utility';
import { getVendors } from './loadDataPurchase';

const PurchaseOrderDatewise = () => {
    const initValues = {
    ...
    const loadPicklistProps = async (token) => {
        ...
        const vendors = await intializeVendor(token);
        setPlVendor(vendors);
    }
    ...
    const loadItemDetails = async (item) => {
     ...
     const loadVendorDetails = async (vendor) => {
        ...
        postValues.vendorID = vendorDetails.vendorID;
        postValues.companyName = vendorDetails.companyName;
        setPostValues({ ...postValues });
    }
    ...
    //Validate Dates and ID
    const validateDatesID = () => {
        ...
        if (postValues.type === "V") {
            if (!postValues.vendorID)
                validationErrors.vendorID = "Select vendor";
        } else {
            if (!postValues.itemID)
                validationErrors.itemID = "Select Item";
        }
    ...
    const getDetails = async () => {
        let fromDate = postValues.fromDate;
        let toDate = postValues.toDate;
        let type = postValues.type;
        let itemID = postValues.itemID;
        let vendorID = postValues.vendorID;
        let validationErrors = validateDatesID();
    ...
    return (
        <Form className="mx-auto col-lg-8 col-md-10 col-sm-12 p-3 formBg">
            <div className="text-center fs-3 mb-1 formHead">VIEW PURCHASE ORDERS DATEWISE</div>
            <Row>
                ...
                <Form.Group className="col-md-6 mb-1" controlId="formType">
                    ...
                    <Form.Select name="type" value={postValues.type} onChange={handleChange} >
                        <option value="V">Vendorwise</option>
                        <option value="I">Itemwise</option>
                    </Form.Select>
                </Form.Group>
                <Form.Group className="col-md-6 mb-3" controlId="formVendor">
                    ...
                    <Form.Text className="text-danger">{errorValues.vendorID}</Form.Text>
                </Form.Group>
                <Form.Group className="col-md-6 mb-3" controlId="formVendorName">
                    <Form.Label>Vendor Name</Form.Label>
                    <Form.Control type="text" name="companyName" value={postValues.companyName} placeholder="Item Name" disabled />
                </Form.Group>
            ...
            <Button type="button" onClick={intialize}>Clear</Button>    
                        ...
                        details.map(item => {
                            return (
                                <tr key={item.purchaseOrderID}>
                                    ...
                                    <td>{formatDateFromDB(item.expectedDate)}</td>
                                    <td>{item.vendorID}</td>
...
export default PurchaseOrderDatewise;